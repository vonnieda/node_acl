  ...
  if (Array.isArray(roles)) {
    var _this = this;
	  ...
	  roles.forEach(function(role) {
	    _this.backend.add(transaction, 'roles', role, userId);
	  });
  }
  else {
	  this.backend.add(transaction, 'roles', roles, userId);
  }
...
Acl.prototype.removeUserRoles = function(userId, roles, cb){
  ...
  if (Array.isArray(roles)) {
    ...
    roles.forEach(function(role) {
      _this.backend.remove(transaction, 'roles', role, userId);
    });
  }
  else {
    this.backend.remove(transaction, 'roles', roles, userId);
  }
...
Acl.prototype.roleUsers = function(roleName, cb){
  return this.backend.getAsync('roles', roleName).nodeify(cb);
};
...
Acl.prototype.hasRole = function(userId, rolename, cb){
  return this.userRoles(userId, function(err, roles) {
...
Acl.prototype.removeRole = function(role, cb){
  ...
  return this.backend.getAsync('resources', role).then(function(resources){
    ...
    _this.backend.del(transaction, 'roles', role)
...
*/
Acl.prototype.middleware = function(numPathComponents, userId, actionsOverride){
  contract(arguments)
    .params()
    .params('number')
